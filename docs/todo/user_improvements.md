...
- **Still open:** PSG still emits an absolute `Cxx` on every note (same baking idea would apply); `analyze.py` skeleton volumes use TL only, not pan; un-audited configs were tuned against the old linear law, so their `Cxx`-carrying notes moved by up to a few dB — re-measure when each is audited.

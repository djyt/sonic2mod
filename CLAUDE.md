...
- When `synthesis.enabled: true`: FM samples auto-generated by `generate_fm_samples()` from voice_instrument_map (root-based), voice_map (C5/C1 fallback), and rootless channel_instrument_map entries
...
- `synth_note_idx = entry.low - 12` — YM2612 synthesizes at the SMPS source note frequency (e.g. G5 = 784 Hz)
- `target_rate = round(amiga_clock / PERIOD_TABLE[entry.root.value])` — links MOD root period to playback rate

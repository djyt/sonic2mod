...
- **Left:** **Invincibility** (64) and **Stage Clear** (22; one instrument shared across transpositions — the audit shows 78/100 with PSG1's ±200 c warble split into notes), **Star Light** (24) and **Continue** (31) scattered, **Spring Yard** 6, **Scrap Brain** 10. **Credits** has no config: it needs `smpsSetTempoDiv` (per-track duration divider, parsed but not applied), durations with GCD 1 at a 58.8 Hz tick rate (1 tick per row would need BPM 294 — 2 ticks per row with `EDx` for the odd ticks), and six tempo steps.
